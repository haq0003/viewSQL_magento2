SELECT 
    ...
    (SELECT 
            value
        FROM
            bddname.dpx_catalog_product_entity_varchar AS `e_var_titre`
        WHERE
            `e_var_titre`.`entity_id` = `e`.`entity_id`
                AND `e_var_titre`.attribute_id IN (SELECT 
                    attribute_id
                FROM
                    `bddname`.`dpx_eav_attribute`
                WHERE
                    `attribute_code` = 'name'
    ...
    (SELECT 
            CONCAT('/', value, '.html')
        FROM
            bddname.dpx_catalog_product_entity_varchar AS `e_var_url`
        WHERE
            `e_var_url`.`entity_id` = `e`.`entity_id`
                AND `e_var_url`.attribute_id IN (SELECT 
                    attribute_id
                FROM
                    bddname.dpx_eav_attribute AS `ea`
                WHERE
    ...
    (SELECT 
            e_dec_price.value
        FROM
            `bddname`.`dpx_catalog_product_entity_decimal` AS `e_dec_price`
        WHERE
    ...
    (SELECT 
        ...
        FROM
            `bddname`.`dpx_catalog_product_entity_decimal` AS `e_dec_spec_price`
        WHERE
    ...
    (SELECT 
            IF(`e_dec_status`.value = 1,
                    ...
                    'inactif')
        FROM
            `bddname`.`dpx_catalog_product_entity_int` AS `e_dec_status`
        WHERE
    ...
    (SELECT 
            CONCAT('https://www.xxxxxxxx.com/catalog/product',
                        e_var_image.value)
        FROM
            `bddname`.`dpx_catalog_product_entity_varchar` AS `e_var_image`
        WHERE
    ...
    (SELECT 
            GROUP_CONCAT(CONCAT('http://www.xxxxxxxx.com/catalog/product',
                            e_mg_gallery.value))
        FROM
            `bddname`.`dpx_catalog_product_entity_media_gallery` AS `e_mg_gallery`
                LEFT JOIN
            `bddname`.`dpx_catalog_product_entity_media_gallery_value_to_entity` AS `e_mg_gallery_ve` ON `e_mg_gallery_ve`.`value_id` = `e_mg_gallery`.`value_id`
        WHERE
    ...
    (SELECT 
        ...
        FROM
            `bddname`.`dpx_catalog_product_entity_text` AS `e_dec_desription`
        WHERE
    ...
    e.updated_at
FROM
    `bddname`.`dpx_catalog_product_entity` AS `e`
        LEFT JOIN
    `bddname`.`dpx_catalog_product_entity_int` AS `e_int_status` ON (`e_int_status`.`entity_id` = `e`.`entity_id`
